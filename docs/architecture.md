...
인증에는 실제 터미널을 상속시키고, 실행·시험·무효화에는 출력 크기가 제한된 파이프(표준 입력은 무시)를 사용합니다. 모두 같은 Pi 프로세스를 부모로 하는 non-detached 직접 자식이므로 같은 sudo timestamp 범위를 사용하는 구성을 시험할 수 있지만, sudoers 정책이나 OS 캐시 구성을 강제하지는 않습니다.
...
stateDiagram-v2
    ...
    Pending --> Granted: -k, -v, -n true 성공 + generation 일치
